...
ICS_Game::go(std::string title, int width, int height, bool fixedSize, bool fullScreen)
{
	...
	// when the loop is complete, destroy the window
	destroyWindow();
...
ICS_Game::createWindow(std::string title, int width, int height, bool fixedSize)
{
	...
	if (!(_deviceContextHandle=GetDC(_windowHandle)))							// Did We Get A Device Context?
	{
		destroyWindow();								// Reset The Display
	...
	if (!(PixelFormat=ChoosePixelFormat(_deviceContextHandle,&pfd)))	// Did Windows Find A Matching Pixel Format?
	{
		destroyWindow();								// Reset The Display
	...
	if(!SetPixelFormat(_deviceContextHandle,PixelFormat,&pfd))		// Are We Able To Set The Pixel Format?
	{
		destroyWindow();								// Reset The Display
...
ICS_Game::destroyWindow()
{
	...
	/*
		// check if there is a render context
		if (_renderContextHandle)
		{
			// attempt to release the device and render contexts
			if (wglMakeCurrent(NULL, NULL))
			{
				// attempt to delete the render context
				if (wglDeleteContext(_renderContextHandle))
				{
					_renderContextHandle = NULL;
				}
			}
	...
	*/
	...
	if (!UnregisterClass(L"OpenGL", _moduleHandle))
	{
		_moduleHandle = NULL;
	}
}