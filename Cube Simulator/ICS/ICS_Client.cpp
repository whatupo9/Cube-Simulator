...
ICS_Client::connectToServer()
{
	...
	if (getaddrinfo(_serverAddress.c_str(), _serverPort.c_str(), &hints, &_addressInfo) != 0)
	{
		_serverAddress = "";
		_serverPort = "";
	...
	for (addrinfo* ptr = _addressInfo; ptr != NULL && _socket == INVALID_SOCKET; ptr = ptr->ai_next)
	{
