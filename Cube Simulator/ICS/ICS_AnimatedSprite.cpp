...
ICS_AnimatedSprite::~ICS_AnimatedSprite()
{
	...
	for (map<string, ICS_Animation*>::iterator iterator = _animations.begin(); iterator != _animations.end(); iterator++)
	{
...
ICS_AnimatedSprite::addAnimation(string name, vector<ICS_Pair<int>> frames, int framesPerSecond, bool loop)
{
	...
	_animations[name] = new ICS_Animation(frames, framesPerSecond);
}
